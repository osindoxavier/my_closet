...
class GetSignInWithEmailPasswordUseCase @Inject constructor(private val repository: AuthenticationRepository) {

    companion object {
        private const val TAG = "GetSignInWithEmailPassw"
    }

    operator fun invoke(request: AuthenticationRequest): Flow<Resource<AuthResult>> = flow {
        try {
            emit(Resource.Loading())
            val result = repository.signIn(request = request)
